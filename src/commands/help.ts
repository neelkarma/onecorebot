...
export = <Command>{
  ...
  execute: async (message, args, client) => {
    ...
    if (!client.commands.has(args[0])) return await message.channel.send(`That command doens't exist! Use \`${process.env.BOT_PREFIX}help\` by itself for a full list of commands.`);
    const helpCommand = client.commands.get(args[0])!;
    return await message.channel.send(`\`${process.env.BOT_PREFIX}${helpCommand.name}\` - *${helpCommand.description}*\n__**Usage:**__\n${helpCommand.usage}`)
  },
};
